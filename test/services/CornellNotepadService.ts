...
import { IDeleteNotesRequest, IDeleteNotesResponse, IDeleteUserRequest, IDeleteUserResponse, IGetNoteRequest, IGetNoteResponse, IGetNotesRequest, IGetNotesResponse, IGetUserRequest, IGetUserResponse, IPostAuthSignInRequest, IPostAuthSignInResponse, IPostAuthSignUpRequest, IPostAuthSignUpResponse, IPostNotesRequest, IPostNotesResponse, IPutNoteRequest, IPutNoteResponse, IPutUserInfoRequest, IPutUserInfoResponse, IPutUserPasswordRequest, IPutUserPasswordResponse } from "../types/cornellNotepadService/types";
...
export class CornellNotepadService {
    ...
    static async getDocsJson(): Promise<Response> {
        let response = await fetch(`${this.endpoint}/docs/swagger.json`)
        return response
    }

    static async signUp<ExpectedResponse = IPostAuthSignUpResponse>(
        requestParams: IPostAuthSignUpRequest,
        expectedStatus: number = 204,
        expectedResponseTypeName: string = "IPostAuthSignUpResponse"
    ): Promise<ExpectedResponse> {
        let response = await this.client['/auth/sign-up'].post(requestParams)
        Assert.equal(response.status, expectedStatus)
        if (expectedResponseTypeName !== "IPostAuthSignUpResponse") {
            return getValidatedResponseBody(
                response,
                this.typesFile,
                expectedResponseTypeName
